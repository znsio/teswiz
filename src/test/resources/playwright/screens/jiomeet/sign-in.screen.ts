...
const LOCATORS = {
  ...
  welcomeBackImage: "img[class*='signin-banner']",
  username: "#username",
...
export async function signIn(screen: ScreenContext, username: string, password: string): Promise<void> {
  await screen.page.locator(LOCATORS.signInLink).click();
  await screen.page.locator(LOCATORS.welcomeBackImage).waitFor();
...
export async function joinAMeeting(
  ...
  await screen.page.locator(LOCATORS.joinButton).click();
