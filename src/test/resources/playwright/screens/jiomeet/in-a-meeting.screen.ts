...
const LOCATORS = {
  meetingInfoIcon: "div.icon.pointer",
  micLabel: "div[class*='mic-section'] img",
  meetingId: "xpath=//div[text()='Meeting ID']/following-sibling::div",
  password: "xpath=//div[text()='Password']/following-sibling::div",
  microphoneButton: "#toggleMicButton div[class*='img-holder']",
} as const;

async function revealMeetingControls(screen: ScreenContext): Promise<void> {
  await screen.page.locator(LOCATORS.meetingInfoIcon).hover();
}

async function readMeetingDetail(screen: ScreenContext, locator: string): Promise<string> {
  const infoIcon = screen.page.locator(LOCATORS.meetingInfoIcon);
  await infoIcon.click();
  const detail = (await screen.page.locator(locator).textContent())?.replace(/\s/g, "") ?? "";
  await infoIcon.click();
  return detail;
}
...
export async function unmute(screen: ScreenContext): Promise<object> {
  await revealMeetingControls(screen);
  await screen.page.locator(LOCATORS.microphoneButton).click();
...
export async function getMicLabelText(screen: ScreenContext): Promise<string> {
  await revealMeetingControls(screen);
  return (await screen.page.locator(LOCATORS.micLabel).textContent())?.trim() ?? "";
}
