...
#  CONFIG=./configs/theapp/theapp_local_ios_config.properties PLATFORM=iOS TAG="@theapp and @invalidLogin1" ./gradlew run
...
#  RUN_IN_CI=true CONFIG=./configs/theapp/theapp_headspin_ios_config.properties CLOUD_KEY=$HEADSPIN_CLOUD_KEY PLATFORM=iOS TAG="@theapp and @invalidLogin1 and @headspin" ./gradlew run
  @android @web @iOS @headspin @browserstack @invalidLogin @invalidLogin1 @theapp
  Scenario: Verify error message on invalid login
    Given I login with invalid credentials - "znsio61", "invalid password"
    Then I try to login again with invalid credentials - "znsio62", "another invalid password"
