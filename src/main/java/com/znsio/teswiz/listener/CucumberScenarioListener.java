...
//        }
    ...
    private static String getReportLinkFromBrowserStack(String sessionId) {
        ...
        String[] curlCommand = new String[] {"curl --insecure " + getCurlProxyCommand() + " -u \"" + cloudUser + ":" + cloudPassword + "\" -X GET \"https://api-cloud.browserstack.com/app-automate/sessions/" + sessionId + ".json\""};
        LOGGER.debug(String.format("Curl command: '%s'", Arrays.toString(curlCommand)));
        CommandLineResponse commandLineResponse = null;
        try {
            commandLineResponse = CommandLineExecutor.execCommand(curlCommand);
            LOGGER.debug(String.format("Response from BrowserStack - '%s'", commandLineResponse.toString()));
