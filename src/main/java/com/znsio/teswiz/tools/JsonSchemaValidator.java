...
public class JsonSchemaValidator {
    ...
    public static JSONObject validateJsonFileAgainstSchema(String jsonFilePath, String jsonContents, String schemaFile) {
        try {
            ...
            LOGGER.debug("Loaded schema file into a json object: " + schemaFile);

            JSONObject jsonObject = new JSONObject(new JSONTokener(jsonContents));
            LOGGER.debug(String.format("Loaded json contents from file '%s' to be validated against the schema into " + "a json object", jsonFilePath));

            Schema schema = SchemaLoader.load(schemaStreamObject);
            schema.validate(jsonObject);
            LOGGER.debug(String.format("Json file '%s' validated successfully against the schema", jsonFilePath));
            return jsonObject;
